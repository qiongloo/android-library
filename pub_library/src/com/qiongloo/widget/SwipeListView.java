...
public class SwipeListView extends ListView {
	 ...
	 * 是否局部侧滑
	 */
	private boolean isPartSlide ;
	/**
	 * 局部侧滑组件id
	 */
	private int partSlideId = 0;
	
	
	
	public boolean isPartSlide() {
		return isPartSlide;
	}

	public void setPartSlide(boolean isPartSlide) {
		this.isPartSlide = isPartSlide;
	}

	public int getPartSlideId() {
		return partSlideId;
	}

	public void setPartSlideId(int partSlideId) {
		this.partSlideId = partSlideId;
	}
	...
	public OnItemClicksListener getListener() {//mod
		return listener;
	}

	public void setListener(OnItemClicksListener listener) {//mod
		this.listener = listener;
	}

	public static interface OnItemClicksListener{//mod
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (motionPosition >= 0) {
				...
				isPartSlide = partSlideId==0?false:true;
				
				if(isPartSlide){//是否局部滑动==新增
					
					View item = currentItemView.findViewById(partSlideId);//===新增
					mPreItemView = mCurrentItemView;
					mCurrentItemView = item;//===新增
 ...
 */
		...
		case MotionEvent.ACTION_CANCEL:
			...
			if (mIsShown
				 ...
				 */
				hiddenRight(mPreItemView);//隐藏上一个view的显示
			}
			
			dx = lastX - mFirstX;//mod
			dy = lastY - mFirstY;//mod
			if (!(Math.abs(dx) >= 5 && Math.abs(dy) >= 5)) {//mod
				if(null!=listener){//mod
					listener.click(pointToPosition((int) ev.getX(), (int) ev.getY()));//mod
				}//mod
			}//mod
